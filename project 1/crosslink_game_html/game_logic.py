...
from path_finder import find_shortest_path, find_path_with_turn_cost, find_path_with_turn_cost_with_cutting, find_shortest_path_with_cutting
...
class Game:
    ...
    def find_all_paths(self, consider_turns=False):
        ...
        print("\n================= 开始寻找所有路径 =================")
        print(f"考虑转向代价: {consider_turns}")
        print(f"棋子对数量: {len(self.pieces)}")
        
        for i, piece in enumerate(self.pieces):
            start_pos = piece["positions"][0]
            end_pos = piece["positions"][1]
            
            print(f"\n正在处理棋子对 {piece['id']} (颜色: {piece['color']}):")
            ...
            print(f"已占用单元格数量: {len(occupied_cells)}")
            
            # 查找路径
            if consider_turns:
                # 使用带剪枝的转向代价搜索算法
                path = find_path_with_turn_cost_with_cutting(start_pos, end_pos, occupied_cells, self.board.size)
                # 对比使用不带剪枝的算法
                print("\n【对比】使用不带剪枝的转向代价算法:")
                path_no_cutting = find_path_with_turn_cost(start_pos, end_pos, occupied_cells, self.board.size)
            else:
                # 使用带剪枝的普通搜索算法
                path = find_shortest_path_with_cutting(start_pos, end_pos, occupied_cells, self.board.size)
                # 对比使用不带剪枝的算法
                print("\n【对比】使用不带剪枝的算法:")
                path_no_cutting = find_shortest_path(start_pos, end_pos, occupied_cells, self.board.size)
